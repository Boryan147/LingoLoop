...
const ExpressionManager: React.FC<ExpressionManagerProps> = ({ items, onUpdate }) => {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto h-full flex flex-col">
      ...
      {/* Add Form */}
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-indigo-100 mb-8 relative overflow-hidden">
        ...
        <form onSubmit={handleAdd} className="flex gap-4 items-start">
            <div className="flex-1">
                ...
                <input
                    ...
                    className="w-full text-lg p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-slate-300"
                    disabled={isLoading}
                />
            ...
            <button
                ...
                className="mt-6 px-6 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all shadow-md shadow-indigo-200"
            >
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-4 pb-20">
        {items.length === 0 ? (
            ...
            <div key={item.id} className="bg-white p-6 rounded-2xl border border-slate-200 hover:border-indigo-300 transition-colors group">
                <div className="flex justify-between items-start mb-3">
                    <h3 className="text-xl font-bold text-slate-800">{item.expression}</h3>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] px-2 py-1 bg-slate-100 rounded text-slate-500 font-mono">
                          Level {item.repetition}
                      </span>
                    </div>
                </div>
                <p className="text-slate-600 italic mb-4 border-l-2 border-indigo-200 pl-3">{item.definition}</p>
                     ...
                     <div className="pt-2 border-t border-slate-200">
                        ...
                        <ul className="list-disc pl-4 mt-1 space-y-1">
                          {item.examples.map((ex, i) => (
                            <li key={i} className="text-sm text-slate-700">{highlightMatch(ex, item.expression)}</li>
                          ))}
                        </ul>
                     </div>
                </div>
